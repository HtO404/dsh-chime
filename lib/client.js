window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function saveSettings() {
      ...
      putPrefs(); // shared host-side source of truth across environments
    }

    // Unlock audio on the first user gesture (Electron / autoplay policies).
    ...
    function fetchPrefs() {
      return fetch("/dsh-chime/prefs").then(function (r) { return r.json(); }).catch(function () { return null; });
    }
    function putPrefs() {
      return fetch("/dsh-chime/prefs", {
        method: "PUT",
        headers: { "content-type": "application/json" },
        body: JSON.stringify({ volume: sound.volume, muted: sound.muted, presetId: sound.presetId, dataUrl: sound.dataUrl, name: sound.name }),
    ...
    function playDefaultChime(count, vol) {
      var actx = ensureCtx();
      if (!actx) {
        setError("当前环境不支持 Web Audio，无法播放提示音");
        return;
      }
      try {
        resumeIfNeeded(actx);
    ...
    function apply(ctx) {
      ...
      // shared host-side prefs are the cross-environment source of truth
      fetchPrefs().then(function (res) {
        var p = res && res.prefs && typeof res.prefs === "object" ? res.prefs : null;
        if (p) {
          if (typeof p.volume === "number" && p.volume >= 0 && p.volume <= 100) sound.volume = p.volume;
          if (typeof p.muted === "boolean") sound.muted = p.muted;
          if (typeof p.presetId === "string" && p.presetId) sound.presetId = p.presetId;
          if (typeof p.dataUrl === "string" && p.dataUrl) {
            sound.dataUrl = p.dataUrl;
            sound.name = (typeof p.name === "string" && p.name) ? p.name : "自定义声音";
          } else {
            sound.dataUrl = null;
            sound.name = null;
          }
      ...
      fetchPresets().then(function (res) {
        ...
        setError("无法连接 Host 路由（/dsh-chime/presets），插件可能未正确挂载");
      });
